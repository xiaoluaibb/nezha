#!/bin/bash
# Nezha Dashboard 一键完整安装脚本（Docker + Nginx + SSL + 安全版）
# 支持单个域名 + Cloudflare / 腾讯 CDN

set -e

echo "=== 哪吒监控 Dashboard 一键完整安装脚本（安全优化版） ==="

# 检查必要组件
echo "检查并安装必要组件..."
apt-get update -qq || yum update -q
apt-get install -y curl wget git tar nginx socat || yum install -y curl wget git tar nginx socat
...
# 检查是否已安装
if docker ps | grep -q nezha; then
...
server {
    ...
    ssl_certificate /etc/nginx/ssl/DOMAIN_PLACEHOLDER.crt;
...
sed -i "s/DOMAIN_PLACEHOLDER/$DOMAIN/g" /etc/nginx/sites-available/nezha
...
if [[ "$cron_choice" == "y" || "$cron_choice" == "Y" ]]; then
    ...
    echo "$cron root cd /opt/nezha && ./nezha.sh install" > /etc/cron.d/nezha-update
    echo "定时任务已添加！"
fi

echo "=== 安装完成！ ==="
echo "面板地址: https://$DOMAIN"
echo "Agent 推荐使用: 服务器IP:8008 （不要走域名）"
echo "强烈建议立即修改后台密码！"
